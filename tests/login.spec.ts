...
test.describe('Login and Dashboard Tests', { tag: ['@login'] }, () => {
    ...
    test('@001 Verify user can log in with valid credentials', async ({ loginPage, page }) => {
        await loginPage.login(Constants.validUsername, Constants.validPassword);
        await expect(page).toHaveURL(/dashboard/);
    });

    test('@002 Verify Dashboard is accessible after login', async ({ loginPage, page }) => {
        await loginPage.login(Constants.validUsername, Constants.validPassword);
        await expect(page).toHaveURL(/dashboard/);
        await expect(loginPage.dashboardHeader).toBeVisible();
    });

    test('@003 Verify error message is displayed when login fields are empty', async ({ loginPage }) => {
    ...
    test('@004 Verify login fails with long input strings', async ({ loginPage }) => {
        const longString = Generate.generateRandomString(256);
        await loginPage.login(longString, longString);
        await expect(loginPage.errorMessage).toBeVisible();
    });

    test('@005 Verify login fails with invalid credentials', async ({ loginPage }) => {
        await loginPage.login('invalidUser', 'invalidPass');
        await expect(loginPage.errorMessage).toContainText('Invalid credentials');
    });

    test('@006 Verify user cannot access Dashboard without logging in', async ({ page }) => {
